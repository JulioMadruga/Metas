...
$cookie = $result_cont[0][6];
 ...
 $consulta_baton= $conn->prepare("select b.VENDEDOR,a.Nome_parceiro, a.id from $mes a, $meta b where a.Material in ($cookie) and a.VENDEDOR = b.rca and a.VENDEDOR = '$vendedor' and a.Quantidade>0 GROUP BY a.id");
 $consulta_baton->execute(array('id' => $id));
 $result_baton= $consulta_baton->fetchAll(); 

 
 $consulta_tri= $conn->prepare(" select vendedor, posit_cookies from $meta order by vendedor");
...
(select id from (SELECT a.VENDEDOR, a.ID, b.Nome,a.MATERIAL IN ($cookie)AS baton, a.Quantidade 
FROM $mes a, clientes b where a.vendedor = b.rca AND a.ID=b.Cod_Cliente and a.MATERIAL IN ($cookie) 
