...
$consulta_cood = $conn->prepare("SELECT super FROM `usuarios` GROUP by super ");
...
foreach ($result_cood as $key=>$value){
    ...
    if (strlen($value['super']) !== 0){
     ...
     $cood = $value['super'];
        ...
        $consulta_geral = $conn->prepare("SELECT * from `ranking` WHERE super = '$cood' ORDER by nome");
    ...
    if (strlen($value['super']) !== 0){
       ...
       // echo "<h1 style='background: '>".$value['super']."</h1>";
                ...
                echo '
                      ...
                      <thead>
                      <tr>
                      <th class="marrom branco" style="text-align: center" colspan="5">MIX IDEAL CHOCOLATE</th>
        ...
        foreach ($result_geral[$z] as $row){
            ...
            echo '<tr>';
                   ...
                   echo  "<th class='azul' scope='row'>$row[2]</th>
