...
sleep(0.25);
 ...
 //------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    $cli_sap = $conn->prepare("SELECT cod_cli, cod_canal FROM `clientes_sap`");
    $cli_sap ->execute();
    $result_cli_sap  = $cli_sap ->fetchAll();

    foreach ($result_cli_sap as $value){

        $up_pedflexx = $conn->prepare("UPDATE `ped_flexx` set cod_canal = $value[1] where cod_cli = $value[0] ORDER BY `ped_flexx`.`data` DESC LIMIT 250");
        $up_pedflexx  ->execute();
...
echo $segunda = "2020-03-25";
echo $sexta = "2020-03-30";
