...
if (isset($_POST['submit'])) {
   ...
   $limpa->execute();
    ...
    /*if (is_uploaded_file($_FILES['filename']['tmp_name'])) {
       ...
       $import55 = $conn->prepare("SET GLOBAL local_infile = 1");
       $import55->execute();
       ...
       $import= $conn->prepare("LOAD DATA LOCAL INFILE '$nome'
          ...
          (ID, Nome_parceiro, CNPJ, VENDEDOR, Material,Texto_breve_do_produto,Quantidade,UM,Endereco,Cidade,Rg,CEP,Data_doc,N_NF,Liquido,Bruto,Un,@var1)
           set Valor_total = REPLACE(trim(@var1), ' ', '.') ");
        // var_dump($import);
       $import->execute();
       $import->closeCursor();
        ...
        $delete = "DROP TABLE IF EXISTS top_$tabMes; DROP TABLE IF EXISTS hierarquia_$tabMes; DROP TABLE IF EXISTS hierarquia_bisc_$tabMes;"; //Esvaziar a tabela
        ...
        $import23= $conn->prepare("CREATE TABLE top_$tabMes SELECT b.hierarquia,  a.* from $tabMes a, clientes b where a.ID = b.cod_cli");
        // var_dump($import23);
        $import23->execute();
        $import23->closeCursor();

        $import24= $conn->prepare("CREATE table hierarquia_$tabMes 
		SELECT vendedor,COUNT(hierarquia) as total, 'as5' as hierarquia FROM (SELECT * FROM top_$tabMes where hierarquia in (5534707,5534712,5534714) and Material in (12415774,12356998,12331542,12312312,11320198,11320367,12416237,12396888,12396921) GROUP by id, Material)sub GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'as34' as hierarquia from (SELECT * FROM top_$tabMes where hierarquia in (5534706) and Material in (12415774,12356998,11320198,11320331,11320367,12416237,12396888) GROUP by id, Material)sub GROUP by hierarquia, vendedor UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'as12' as hierarquia FROM (SELECT * FROM top_$tabMes where hierarquia in (5534705) and Material in (12415774,12356998,11320198,11320331,11320367,12416237,12396888) GROUP by id, Material)sub GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'trad' as hierarquia FROM (SELECT * FROM top_$tabMes where hierarquia in (5534716) and Material in (11320198,11320367,11320331,12416237) GROUP by id, Material)sub GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'conv' as hierarquia FROM (SELECT * FROM top_$tabMes where hierarquia in (5534708) and Material in (11320198,11320367,11320331,12416237) GROUP by id, Material)sub GROUP by hierarquia, vendedor");
        //var_dump($import24);
        $import24->execute();
        $import24->closeCursor();

        $import25= $conn->prepare("CREATE table hierarquia_bisc_$tabMes SELECT vendedor,COUNT(hierarquia) as total, 'as5' as hierarquia FROM top_$tabMes where hierarquia in (5534707,5534712,5534714) and Material in (12365150,12365129,12365282,12365128) GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'as34' as hierarquia FROM top_$tabMes where hierarquia in (5534706) and Material in (12365150,12365129,12365128) GROUP by hierarquia, vendedor UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'as12' as hierarquia FROM top_$tabMes where hierarquia in (5534705) and Material in (12365150,12365129) GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'trad' as hierarquia FROM top_$tabMes where hierarquia in (5534716) and Material in (12365150) GROUP by hierarquia, vendedor  UNION ALL
        SELECT vendedor,COUNT(hierarquia) as total, 'conv' as hierarquia FROM top_$tabMes where hierarquia in (5534708) and Material in (12365150) GROUP by hierarquia, vendedor");
       // var_dump($import25);
        $import25->execute();
        $import25->closeCursor();
