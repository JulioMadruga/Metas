...
if(!isset($_GET['regiao'] )|| $regiao == 'Todos'){
    ...
    $consulta_baton = $conn->prepare("SELECT rca, trimarca, COUNT(NOME_parceiro) as realizado, if(trimarca - COUNT(NOME_parceiro)<0,0,trimarca  - COUNT(NOME_parceiro)) as dif FROM (SELECT b.rca, a.NOME_PARCEIRO, b.rca as vend, b.trimarca  FROM $mes a, $meta b where a.MATERIAL IN ($bat) AND a.QUANTIDADE>0 and a.vendedor = b.rca group by a.id)SUB GROUP BY rca");
    $consulta_baton->execute();
    $result_baton = $consulta_baton->fetchAll();


    $consulta_baton2 = $conn->prepare(" select rca, trimarca from $meta order by vendedor");
    $consulta_baton2->execute(array('id' => $id));
    $result_baton2 = $consulta_baton2->fetchAll();
...
<table align="center" cellpadding="5"> 
           <tr style="background-color: #84071D; color: #FFFFFF; font-weight: bold; font-family:Dosis; " >
               <td colspan="4" style="width: 680px; height: 90px; text-align: center; font-size: 25px;background: white;"><img src="../images/jumbo.png"></td>
           </tr>
</table>
           </div>
<div class="col-md-1" style="background: #CED4D6; height: 90px;"></div>
