...
if(isset($_GET['regiao'])) {
    ...
    $consulta_baton = $conn->prepare("SELECT VENDEDOR, posit_cookies, COUNT(NOME_parceiro) as realizado, if(posit_cookies - COUNT(NOME_parceiro)<0,0,posit_cookies - COUNT(NOME_parceiro)) as dif  FROM (SELECT b.VENDEDOR, a.NOME_PARCEIRO, b.vendedor as vend, b.posit_cookies 
FROM $mes a, $meta b, usuarios c where a.MATERIAL IN ($jum) AND a.QUANTIDADE>0 and a.vendedor = b.rca and b.rca = c.rca and c.regiao = '$regiao' group by a.nome_parceiro)SUB GROUP BY VENDEDOR");
    ...
    $consulta_baton2 = $conn->prepare(" select a.vendedor, a.posit_cookies, a.rca from $meta a, usuarios b where a.rca = b.rca and b.regiao = '$regiao' order by vendedor");
    $consulta_baton2->execute(array('id' => $id));
    $result_baton2 = $consulta_baton2->fetchAll();

// var_dump($result_baton2);

    $consulta_TOTAL = $conn->prepare("Select sum(posit_cookies), sum(realizado), IF(sum(posit_cookies) - sum(realizado)<0,0,sum(posit_cookies) - sum(realizado)) AS DIF from 
(SELECT VENDEDOR, posit_cookies, COUNT(NOME_parceiro) as realizado, if(posit_cookies - COUNT(NOME_parceiro)<0,0,posit_cookies - COUNT(NOME_parceiro)) as dif  FROM 
(SELECT b.VENDEDOR, a.NOME_PARCEIRO, b.vendedor as vend, b.posit_cookies FROM $mes a, $meta b, usuarios c where
...
if(!isset($_GET['regiao'] )|| $regiao == 'Todos') {
    ...
    $consulta_baton = $conn->prepare("SELECT VENDEDOR, posit_cookies, COUNT(id) as realizado, if(posit_cookies - COUNT(id)<0,0,posit_cookies - COUNT(id)) as dif  FROM 
(SELECT b.VENDEDOR, a.NOME_PARCEIRO,a.id, b.vendedor as vend, b.posit_cookies FROM $mes a, $meta b where 
a.MATERIAL IN ($jum) AND a.QUANTIDADE>0 and a.vendedor = b.rca group by a.id)SUB GROUP BY VENDEDOR");
   ...
   // print_r($consulta_baton);
    ...
    $consulta_baton2 = $conn->prepare(" select vendedor, posit_cookies, rca from $meta order by vendedor");
    $consulta_baton2->execute(array('id' => $id));
    $result_baton2 = $consulta_baton2->fetchAll();
    ...
    $consulta_TOTAL = $conn->prepare("Select sum(posit_cookies), sum(realizado), IF(sum(posit_cookies) - sum(realizado)<0,0,sum(posit_cookies) - sum(realizado)) AS DIF from 
( SELECT VENDEDOR, posit_cookies, COUNT(id) as realizado, if(posit_cookies - COUNT(id)<0,0,posit_cookies - COUNT(id)) as dif  FROM 
(SELECT b.VENDEDOR, a.NOME_PARCEIRO, a.id, b.vendedor as vend, b.posit_cookies FROM $mes a, $meta b where 
        ...
        <ul class="nav">
            ...
            <li class="button-dropdown"><a href="javascript:void(0)" class="dropdown-toggle">  Positivações  <span>▼</span>  </a>
                <ul class="dropdown-menu">
                    ...
                    <li  role="presentation" id="active"><a href="#">Cookies</a></li>
            ...
            <?php
            if($usuario->getNome()=="Julio" || $usuario->getNome()=="Marciano"){?>
...
<table align="center" cellpadding="5"> 
           <tr style="background-color: #9A6A0B; color: #FFFFFF; font-weight: bold; font-family:Dosis; " >
               <td colspan="4" style="width: 680px; height: 90px; text-align: center; font-size: 25px;background: #FEFBED;"><img src="../images/cookie.jpg"></td>
           </tr>
</table>
           </div>
<div class="col-md-1" style="background: #CED4D6; height: 90px;"></div>
    ...
    foreach($result_baton2 as $row) {
        ...
        extract($row);
                ...
                $verfic = true;
                           ...
                           echo'<td class="link" style="width: 150px; text-align: center; border: solid; border-color: #245269;"><a id href="cookie.php?mes='.$meta.'&vd='.$row[2].'">'.$row[0].'</a></td>';
