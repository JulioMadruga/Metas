...
$rech = $result_cont[0][5];

 $consulta_baton= $conn->prepare("select b.VENDEDOR,a.Nome_parceiro, a.id from $mes a, $meta b where a.Material in ($rech) and a.VENDEDOR = b.rca and a.VENDEDOR = '$vendedor' and a.Quantidade>0 GROUP BY a.id");
 $consulta_baton->execute(array('id' => $id));
 $result_baton= $consulta_baton->fetchAll(); 

 
 $consulta_tri= $conn->prepare(" select vendedor, posit_rech from $meta order by vendedor");
...
(select id from (SELECT a.VENDEDOR, a.ID, b.Nome,a.MATERIAL IN ($rech)AS baton, a.Quantidade 
FROM $mes a, clientes b where a.vendedor = b.rca AND a.ID=b.Cod_Cliente and a.MATERIAL IN ($rech) 
...
<h2 style="text-align:center;"class="btn btn-default btn-lg link2"><a style="padding:6px;"href="rech.php">Voltar</a><h2>
</div>
