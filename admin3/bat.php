...
$consulta_baton= $conn->prepare("select id,vendedor, razao from (SELECT a.VENDEDOR, a.ID, b.razao,a.MATERIAL IN ($bat)AS baton, a.Quantidade FROM $mes a, clientes b where a.vendedor = b.rca AND a.ID=b.Cod_Cli and a.MATERIAL IN ($bat) and a.VENDEDOR=$vendedor)sub GROUP by id");
$consulta_baton->execute(array('id' => $id));
$result_baton= $consulta_baton->fetchAll();


$consulta_cli= $conn->prepare("SELECT Vendedor, Cod_Cli, razao FROM clientes WHERE rca = '$vendedor' order by razao");
...
$consulta_positivar= $conn->prepare("SELECT Vendedor,Cod_Cli,razao from clientes WHERE Cod_Cli NOT IN 
(select id from (SELECT a.VENDEDOR, a.ID, b.razao,a.MATERIAL IN ($bat)AS baton, a.Quantidade 
FROM $mes a, clientes b where a.vendedor = b.rca AND a.ID=b.Cod_Cli and a.MATERIAL IN ($bat) 
