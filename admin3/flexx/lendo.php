...
// Abre o Arquvio no Modo r (para leitura)
        ...
        $insert_cli = $conn->prepare("TRUNCATE TABLE clientes_flexx");
        // var_dump($insert_cli);
        $insert_cli->execute();
...
// Lê o conteúdo do arquivo
        while (!feof($arquivo)) {
            ...
            If (substr($linha, 0, 1) == '|' && substr($linha, 0, 2) !== '| ') {
                $result = explode("|", $linha);

               // var_dump($result[1]);

               // var_dump(intval(substr($result[1], 1, 7)));
                //var_dump(intval($result[1]));
               $cod = trim(substr($result[1], 0, 7));
               $razao = trim(substr($result[1], 11, 40));
                if(trim(substr($result[1], 49, 5)) == "5-B"){
                    $vendedor = "5"."<br>";
                }else{
                   $vendedor = trim(substr($result[1], 49, 5));
                }

                $canal = trim(utf8_encode(substr($result[1], 87, 20)));



               if($vendedor <> 999 && $vendedor <> 995 && $vendedor <> 888 && $vendedor <> 444 && $vendedor <> 998 && $vendedor <> 333 && $vendedor <> ''){


                   $insert_cli = $conn->prepare("INSERT INTO clientes_flexx(cod_cli,razao,rca,canal) values ('$cod','$razao','$vendedor','$canal')");
                  // var_dump($insert_cli);
                   $insert_cli->execute();



                   //echo $cod ." -- ".$razao ." -- ".$vendedor ." -- ".$canal."<br>";
               }
...
//
//
//                $consulta_rca= $conn->prepare("SELECT rca FROM clientes where cod_cli = $cod_cli");
//                // var_dump($consulta_nf);
